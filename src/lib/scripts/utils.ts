...
declare global {
  type TypeValidator = [
    (val: string, ...payload: TypeValidatorPayload[]) => boolean,
    any?
  ];
}
...
export const getRoomDetails = (
  ...
  if (hotelsDetails && activeHotelId) {
    activeHotelDetails = hotelsDetails.find(
...
export const getCCNumberMaskType = (
  value: string
): TypeMaskOption | undefined => {
